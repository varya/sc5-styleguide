var path = require('path'),
  ...
  kssParser = require('./kss-parser'),
  angularFiles = require('./angular-files'),
  wrapperMarkup = require('./wrapper-markup'),
  sectionReferences = require('./section-references'),
...
function emitProgressStart() {
  if (socketIsOpen()) {
    process.env['STYLEGUIDE_SERVER_INST'].io.emitProgressStart();
  }
}

function emitProgressEnd() {
  if (socketIsOpen()) {
    process.env['STYLEGUIDE_SERVER_INST'].io.emitProgressEnd();
  }
}

function emitCompileError(error) {
  if (socketIsOpen()) {
    process.env['STYLEGUIDE_SERVER_INST'].io.emitCompileError(error);
  }
}

function emitCompileSuccess() {
  if (socketIsOpen()) {
    process.env['STYLEGUIDE_SERVER_INST'].io.emitCompileSuccess();
  }
}
        ...
        Q.all([stylesCompiled, overviewProcessed, filesCopied, indexHtmlProcessed])
          .then(function() {
            if (opt.server) {
              ...
              process.env['STYLEGUIDE_SERVER_INST'] = startServer(opt);
            }
            emitCompileSuccess();
          })
